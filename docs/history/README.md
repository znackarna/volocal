...
**[2026-08-14](2026-08-14.md)** — Settings regrouped after its owner looked at yesterday's three tabs and said they do not help: `Vzhled` and `Aktualizace` stand on their own again, `O aplikaci` is `Informace`, and what was left under `Aplikace` is named for what is actually in it — `Složky a zálohy`. Only the grouping changed; every control the simplification removed stays removed.
